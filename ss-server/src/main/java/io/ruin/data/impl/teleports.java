...
public class teleports extends DataFile {
    ...
    public static void chartteleport(Player player, int x, int y, int z, int o) {
        player.resetActions(true, true, true);
        //player.lock(LockType.FULL_NULLIFY_DAMAGE); //keep lock outside of event!
        if (player.getInventory().hasItem(995, o)) {
            player.lock(LockType.FULL_NULLIFY_DAMAGE); //keep lock outside of event!
            player.startEvent(e -> {
                player.getPacketSender().fadeOut();
                e.delay(2);
                player.getMovement().teleport(x, y, z);
                player.getInventory().remove(995, o);
                player.getPacketSender().clearFade();
                Nurse.heal(player, null);
                PlayerCounter.TELEPORT_PORTAL_USES.increment(player, 1);
                player.getPacketSender().fadeIn();
            ...
            for (int CHART : CHARTERNPC) {
                player.dialogue(new NPCDialogue(CHART, "You need "+o+" coins to travel to that area."));
            }
        }
    }
