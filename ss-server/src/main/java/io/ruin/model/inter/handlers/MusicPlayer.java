    ...
    public static final class Track {
            ...
            try {
                ...
                /* unlock track */
                    ...
                    if (player.isGroup(PlayerGroup.OWNER)) {
                        player.sendMessage("Playing" + archiveId + ". (varpPos=" + varpPos + ", varpShift=" + varpShift + ")");
                    }
          //      }
            } catch(Throwable t) {
        ...
        private void select(Player player) {
            try {
                ...
                player.getPacketSender().sendString(Interface.MUSIC_PLAYER, 5, name);
                player.getPacketSender().sendMusic(archiveId);
                player.sendMessage("playing music");
            } catch(Throwable t) {
                Server.logError("[MusicPlayer] select", t); //todo remove after find bug
            }
        }
