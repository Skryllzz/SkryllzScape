...
public abstract class JournalEntry {
    ...
    protected final void asend(Player player, String text) {
        if (category)
            player.getPacketSender().sendClientScript(135, "ii",701 << 16 | childId, 496);
        player.getPacketSender().sendString(701, childId, text);
    }

    protected final void asend(Player player, String text, Color color) {
        if (category)
            player.getPacketSender().sendClientScript(135, "ii",701 << 16 | childId, 496);
        player.getPacketSender().sendString(701, childId, color.wrap(text));
    }

    protected final void asend(Player player, String key, String value, Color color) {
        if (category)
            player.getPacketSender().sendClientScript(135, "ii",701 << 16 | childId, 496);
        player.getPacketSender().sendString(701, childId, ("<col=D37E2A>" + key + ":</col> " + color.wrap(value)));
    }

    protected final void asend(Player player, String key, int value, Color color) {
        if (value == 0)
            color = Color.RED;
        if (category)
            player.getPacketSender().sendClientScript(135, "ii",701 << 16 | childId, 496);
    ...
    public abstract void asend(Player player);
