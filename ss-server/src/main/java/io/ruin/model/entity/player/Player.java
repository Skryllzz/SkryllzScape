...
public class Player extends PlayerAttributes {
    ...
    public int getClientGroupId() {
       ...
       // }
        if (getGameMode() == GameMode.ULTIMATE_IRONMAN) {
            return getGameMode().ordinal() + 1;
        }
        if (getGameMode() == GameMode.IRONMAN) {
           return getGameMode().ordinal() + 1;
        }
        if (getGameMode() == GameMode.HARDCORE_IRONMAN) {
            return getGameMode().ordinal() + 7;
        }
    ...
    public void init(LoginInfo info) {
        ...
        if(position == null)
            position = new Position(3093, 3105, 0);
        else
    ...
    public void start() {
         ...
         * Messages
         */
        sendMessage("Welcome to " + World.type.getWorldName() + " BETA."); //Server Welcome Message
        ...
        //if (World.isDev())
           // player.join(PlayerGroup.OWNER); //steven delete
    }
