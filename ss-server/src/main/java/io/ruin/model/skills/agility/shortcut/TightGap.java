...
public class TightGap {

    public static void shortcut(Player p, GameObject gap, int levelReq) {
        ...
        p.startEvent(e -> {
            ...
            p.animate(753);
            if(p.getAbsY() >= gap.y)
                p.getMovement().force(2, -3, 0, 0, 0, 60, Direction.SOUTH_EAST);
            else
                p.getMovement().force(-2, 3, 0, 0, 0, 60, Direction.NORTH_WEST);
            e.delay(2);
